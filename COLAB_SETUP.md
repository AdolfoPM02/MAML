...
sigue siendo `eval.py`). Usa `scripts/make_eval_video.py` con un **único rollout
reproducible con seed 42** (`--rollouts 1 --seed 42`). El vídeo se escribe en
...
!env MPLBACKEND=Agg CUDA_VISIBLE_DEVICES="" xvfb-run -a {PY} scripts/make_eval_video.py \
  ...
  --out outputs/best_agent_loop_empty.mp4 --rollouts 1 --seed 42 --device cpu
```
